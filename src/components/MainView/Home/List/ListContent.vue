...
export default defineComponent({
  ...
  components: {
    ...
    AddGameToListDialog,
    ListDialog,
    ListArrangementDialog,
    CollectionRelationDialog,
    RemoveGameDialog
  },
  setup(props) {
    ...
    const games = computed(
      () => store.state.entities.listInServerGames[props.list.list.id] ?? []
    );
    ...
    const deleteList = async () => {
      if (props.list) {
        const res = window.confirm(
          `リスト: 「${props.list.list.name}」を本当に削除しますか？`
        );
        if (res) {
    ...
    const rightCardClick = (title: string) => {
      ...
      if (game) {
        const menu = setupCardMenuList(game.gamename ?? '', async () => {
          await store.dispatch.domain.deleteGamesFromListInServer({
            listId: props.list.list.id,
            gameIds: [game.id]
          });
        });
        menu.popup();
    ...
    const listIcon = computed(() =>
      props.list.list.url?.includes('wishlist')
        ? 'card_giftcard'
        : props.list.list.url
        ? 'link'
        : 'link_off'
